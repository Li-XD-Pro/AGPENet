...
num_pose = 10 # number of poses to visualize per each object-affordance pair
    ...
    for i in range(len(result)):
        if result[i]['semantic class'] == 'Mug':
            ...
            for affordance in shape['affordance']:
                ...
                poses = [
                    create_gripper_marker(color=color_code_2).apply_transform(t)
                    for t in T
                    if np.min(np.linalg.norm(point_cloud.vertices - (t @ np.array([0., 0., 6.59999996e-02, 1.]))[:3],
                                             axis=1)) <= 0.03
                ]
